...
app.post('/api/v1/auth/register', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'INSERT INTO users (email, password, first_name, last_name, phone, city) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, email, first_name, last_name, role',
      [email, hash, firstName, lastName, phone, city]
    );
...
app.post('/api/v1/auth/login', async (req, res) => {
  ...
  const token = jwt.sign({ id: user.id, email: user.email, role: user.role }, JWT_SECRET, { expiresIn: '7d' });
  res.json({ status: 'success', data: { user: { id: user.id, email: user.email, first_name: user.first_name, last_name: user.last_name, role: user.role, phone: user.phone, city: user.city }, token } });
});

app.get('/api/v1/auth/me', authMiddleware, async (req, res) => {
  const result = await pool.query('SELECT id, email, first_name, last_name, phone, city, avatar, role FROM users WHERE id = $1', [req.user.id]);
  res.json({ status: 'success', data: { user: result.rows[0] } });
});

// Produits protégés (création)
app.post('/api/v1/products', authMiddleware, upload.single('image'), async (req, res) => {
  try {
    const { name, price, description, category, location } = req.body;
    const sellerName = req.user.first_name || req.user.email;
    const imageUrl = req.file ? req.file.path : null;
    const result = await pool.query(
      'INSERT INTO products (name, price, description, image, category, location, seller_id, seller_name) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *',
      [name, price, description, imageUrl, category, location, req.user.id, sellerName]
    );
...
app.get('/api/v1/users/:id/products', async (req, res) => {
  const result = await pool.query('SELECT * FROM products WHERE seller_id = $1 ORDER BY created_at DESC', [req.params.id]);
  res.json({ status: 'success', data: { products: result.rows } });
});
