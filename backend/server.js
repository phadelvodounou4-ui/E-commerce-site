...
function computeMarketplaceRank(product, userContext) {
  ...
  if (userContext.search && product.name) { var sl = userContext.search.toLowerCase(); var pt = (product.name + ' ' + (product.description || '')).toLowerCase(); if (pt.indexOf(sl) !== -1) score += 25; else score += 10; } else score += 12;
  var rating = parseFloat(product.seller_rating_avg) || 0; var rc = parseInt(product.seller_rating_count) || 0;
  if (rc >= 10) score += rating * 2; else if (rc >= 5) score += rating * 1.5; else score += 2.5;
...
function getUserContext(query) { return { lat: parseFloat(query.lat) || null, lon: parseFloat(query.lon) || null, search: query.search || '' }; }

// 🔥 SUPPRIME TOUT et recrée VIDE
async function initDB() {
  await pool.query('DROP TABLE IF EXISTS product_saves CASCADE');
  await pool.query('DROP TABLE IF EXISTS product_views CASCADE');
  ...
  // Seulement le compte admin, AUCUN produit
  var hash = await bcrypt.hash('123456', 10);
  await pool.query('INSERT INTO users (email, password, first_name, response_rate, role) VALUES ($1,$2,$3,$4,$5) ON CONFLICT DO NOTHING', ['admin@shop.com', hash, 'Admin', 0.95, 'admin']);
  console.log('✅ Base de données VIDÉE et recréée. Aucun produit simulé.');
}
...
app.get('/api/v1/products', async function(req, res) {
  try {
    ...
    var products = result.rows.map(function(p) { var rank = computeMarketplaceRank(p, context); p.marketplace_rank = rank.final_score; p.distance_km = rank.distance_km; return p; });
