...
const authMiddleware = function(req, res, next) {
  ...
  catch (err) { return res.status(401).json({ status: 'fail' }); }
};
...
function computeMarketplaceRank(p, ctx) {
  var s = 0;
  var d = ctx.lat&&ctx.lon&&p.lat&&p.lon ? haversineDistance(ctx.lat,ctx.lon,p.lat,p.lon) : 999;
  if(d<1)s+=40;else if(d<5)s+=35;else if(d<10)s+=30;else if(d<25)s+=25;else if(d<50)s+=18;else if(d<100)s+=10;else if(d<200)s+=5;else s+=1;
  if(ctx.search&&p.name){var sl=ctx.search.toLowerCase();var pt=(p.name+' '+(p.description||'')).toLowerCase();if(pt.indexOf(sl)!==-1)s+=25;else s+=10;}else s+=12;
  var r=parseFloat(p.seller_rating_avg)||0,rc=parseInt(p.seller_rating_count)||0;if(rc>=10)s+=r*2;else if(rc>=5)s+=r*1.5;else s+=2.5;
  s+=(parseFloat(p.seller_response_rate)||0.5)*5;
  var v=parseInt(p.view_count)||0;if(v>100)s+=4;else if(v>50)s+=3;else if(v>10)s+=2;else s+=1;
  var h=(Date.now()-new Date(p.created_at).getTime())/(36e5);if(h<1)s+=10;else if(h<6)s+=9;else if(h<24)s+=7;else if(h<72)s+=5;else if(h<168)s+=3;else s+=1;
  if(p.image)s+=3;
  return {final_score:Math.round(s),distance_km:d<999?d:null};
}

// 🗑️ Route pour TOUT supprimer
app.get('/api/v1/reset-all-data', async function(req, res) {
  try {
    await pool.query('DELETE FROM product_saves');
    ...
    res.json({ status: 'success', message: 'Toutes les données ont été supprimées. Base vide.' });
  } catch (err) { res.status(500).json({ status: 'error' }); }
});

// Init (garde les tables si elles existent)
...
app.get('/api/v1/products', async function(req, res) {
  var ctx = { lat: parseFloat(req.query.lat)||null, lon: parseFloat(req.query.lon)||null, search: req.query.search||'' };
  ...
  var products = result.rows.map(function(p){var rank=computeMarketplaceRank(p,ctx);p.marketplace_rank=rank.final_score;p.distance_km=rank.distance_km;return p;});
...
app.get('/api/v1/users/:id/products', async function(req, res) { var result = await pool.query('SELECT * FROM products WHERE seller_id=$1 ORDER BY created_at DESC',[req.params.id]); res.json({status:'success',data:{products:result.rows}}); });
...
app.get('/api/v1/messages/:productId', authMiddleware, async function(req, res) { var result = await pool.query('SELECT m.*, u.first_name as sender_name FROM messages m JOIN users u ON m.sender_id=u.id WHERE m.product_id=$1 AND (m.sender_id=$2 OR m.receiver_id=$2) ORDER BY m.created_at ASC',[req.params.productId,req.user.id]); res.json({status:'success',data:{messages:result.rows}}); });
app.post('/api/v1/messages', authMiddleware, async function(req, res) { await pool.query('UPDATE products SET message_count=message_count+1 WHERE id=$1',[req.body.product_id]); var result = await pool.query('INSERT INTO messages (product_id,sender_id,receiver_id,content) VALUES ($1,$2,$3,$4) RETURNING *',[req.body.product_id,req.user.id,req.body.receiver_id,req.body.content]); res.status(201).json({status:'success',data:{message:result.rows[0]}}); });
app.post('/api/v1/ratings', authMiddleware, async function(req, res) { var result = await pool.query('INSERT INTO ratings (seller_id,buyer_id,product_id,score,comment) VALUES ($1,$2,$3,$4,$5) RETURNING *',[req.body.seller_id,req.user.id,req.body.product_id,req.body.score,req.body.comment]); res.status(201).json({status:'success',data:{rating:result.rows[0]}}); });
app.get('/api/v1/cart', async function(req, res) { var result = await pool.query('SELECT c.*, p.name, p.price, p.image FROM cart_items c JOIN products p ON c.product_id=p.id'); var total = result.rows.reduce(function(s,i){return s+parseFloat(i.price)*i.quantity;},0); res.json({status:'success',data:{items:result.rows,total:total.toFixed(2)}}); });
app.post('/api/v1/cart', async function(req, res) { var e = await pool.query('SELECT * FROM cart_items WHERE product_id=$1',[req.body.productId]); if(e.rows.length>0) await pool.query('UPDATE cart_items SET quantity=quantity+1 WHERE product_id=$1',[req.body.productId]); else await pool.query('INSERT INTO cart_items (user_id,product_id,quantity) VALUES (1,$1,1)',[req.body.productId]); res.json({status:'success'}); });
app.delete('/api/v1/cart/:id', async function(req, res) { await pool.query('DELETE FROM cart_items WHERE id=$1',[req.params.id]); res.json({status:'success'}); });
