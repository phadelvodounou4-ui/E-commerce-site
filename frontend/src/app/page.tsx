...
import { Menu, X, Home, ShoppingBag, PlusCircle, User, ShoppingCart, HelpCircle, Settings, Moon, Sun } from 'lucide-react';
...
export default function HomePage() {
  ...
  useEffect(() => {
    // Lire le mode sombre depuis localStorage
  ...
  var secBg = darkMode ? 'bg-[#111]' : 'bg-white';
  ...
  return (
    <div className={`min-h-screen ${bg}`} style={{ fontFamily: 'system-ui, sans-serif' }}>
      <nav className={`sticky top-0 z-40 ${navBg} border-b`}>
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={toggleDarkMode} className="p-2 hover:bg-black/5 rounded-lg">{darkMode ? <Sun size={18} color="#f59e0b" /> : <Moon size={18} />}</button>
      ...
      <section className={`${secBg} border-b border-black/[0.08]`}>
        <div className="max-w-6xl mx-auto px-6 py-16 text-center">
          ...
          <div className="flex gap-3 justify-center mt-6">
            ...
            <Link href="/sell" className={`px-6 py-3 rounded-full border font-semibold ${darkMode?'border-white/20':'border-black/15'}`}>Publier</Link>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 py-10">
        ...
        {loading ? <p>Chargement...</p> : products.length === 0 ? (
          <div className={`${cardBg} rounded-2xl border p-10 text-center`}>
            ...
            <p className="text-lg font-semibold mb-2">Aucune annonce</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {products.slice(0,8).map((p:any) => (
              <Link key={p.id} href={`/products/${p.id}`} className={`${cardBg} rounded-xl border overflow-hidden hover:shadow-lg no-underline`}>
                ...
                <div className="p-3"><h3 className="font-semibold text-sm truncate">{p.name}</h3><span className="text-lg font-extrabold">{p.price} FCFA</span></div>
              </Link>
            ))}
          </div>
        )}
      ...
      <footer className={`${darkMode?'bg-[#111]':'bg-white'} border-t border-black/[0.08] py-8 text-center text-sm ${darkMode?'text-gray-400':'text-[#606770]'}`}>© 2026 MarchéDirect</footer>
    </div>
  );
}
